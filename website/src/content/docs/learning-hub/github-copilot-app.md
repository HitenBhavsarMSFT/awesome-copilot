...
*(v1.0.81+)* Run `copilot app` from anywhere in the terminal to open the GitHub Copilot app in the current directory:

```bash
copilot app
```

This is a shortcut that launches the desktop app and points it at your current working directory, so you can quickly switch between a terminal session and the visual app interface without navigating manually.

In earlier releases (v1.0.79), the `/app` slash command inside a CLI session also opens the **current session** in the desktop app (requires GitHub Copilot app 1.1.3 or later):
