...
*(v1.0.79+)* If you are already in a **Copilot CLI session**, you can open that session directly in the GitHub Copilot app (requires GitHub Copilot app 1.1.3 or later) using the `/app` command:

```
/app    # open the current CLI session in the GitHub Copilot desktop app
```

This is the fastest way to bridge the CLI and the app — start in the terminal, then hand off to the visual interface without losing your session context.
