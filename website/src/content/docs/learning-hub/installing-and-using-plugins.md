...
> **Auto-updates (v1.0.78+)**: First-party plugins (those installed from the official `copilot-plugins` marketplace) automatically update to the latest version at the start of each session. You do not need to run `copilot plugin update` manually for first-party plugins — they are always current when you start Copilot.
