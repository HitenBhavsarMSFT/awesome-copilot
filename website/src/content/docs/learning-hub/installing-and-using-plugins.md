...
**Auto-updating marketplace plugins** *(v1.0.79+)*: Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to automatically update its plugins at the start of each session — no manual `copilot plugin update` required:
...
This is useful for internal marketplaces that release frequently and where teams want to stay on the latest version automatically.
