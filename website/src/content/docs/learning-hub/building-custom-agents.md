...
*(v1.0.81+)* ACP clients now also receive **live updates from subagents**. When the main agent delegates to a subagent (e.g., via `/fleet` or a custom orchestration agent), ACP-connected tools receive streaming progress from those subagents too — not just the top-level agent. This makes ACP integrations much richer for complex multi-agent workflows.
