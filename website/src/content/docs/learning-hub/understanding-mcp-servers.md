...
- *(v1.0.81+)* GitHub Copilot CLI supports the **MCP 2026-07-28 specification**, including support in CLI, SDK, IDE, and in-memory clients
...
- **Windows Authentication Broker (WAM)** *(v1.0.81+, Windows only)*: On Windows, remote MCP servers protected by Microsoft Entra ID can authenticate through the OS authentication broker (WAM). This typically completes with no prompt at all, using your already-signed-in Windows identity. Other platforms, `--device-code` mode, and machines without the broker library use the existing browser flow as a fallback.
