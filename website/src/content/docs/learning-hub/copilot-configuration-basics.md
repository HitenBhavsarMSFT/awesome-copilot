...
**Model family aliases** (v1.0.64+): Instead of typing a full model name, you can use short family aliases in the model setting: `opus`, `sonnet`, `haiku` (Anthropic), and `gpt`, `gemini`, `grok` (Google/OpenAI/xAI). The CLI resolves the alias to the latest available model in that family. This is especially useful in scripts or configuration files where you want to track the best model in a family without hardcoding a version string. Recent models available include **Claude Opus 5** (v1.0.75+), the latest in Anthropic's Opus family for the most demanding tasks, **Gemini 3.7 Flash** and **Grok 4.6** (both v1.0.81+) — use `xhigh` reasoning effort with Grok 4.6 for the highest-quality output.
